...
void CMainFrame::DoLogin(bool SkipLoginWindow, const CString & previousPassword)
{
	...
	m_singleInstance = new CLimitSingleInstance(path.native());
...
void CMainFrame::RestoreState()
{
	...
	for (boost::filesystem::directory_iterator itr(folder); itr != end_itr; ++itr)
	{
		...
		{
			if (boost::algorithm::iequals(itr->path().extension().string(), ".xml"))
				m_persistedWindows.push_back(new CPersistedItem(this, (const char *)CT2A(itr->path().native().c_str())));
		}
	}
