		...
		for (int i = 9; i > 0; --i)
		{
			...
			if (boost::filesystem::exists(bakFilepath))
			{
				try {
					...
					_DBGLOG(LEVEL_WARNING, bakFilepath.native().c_str());
				}
			}
...
public:
	...
	void UpdateID()
	{
		...
		m_folder = m_environmentFolder.native();
...
unsigned int GetWorkspaces(IRepository * repository, IWorkspaceVector * results, bool attributesOnly)
{
	try
	{
		...
		for (boost::filesystem::directory_iterator itr(environmentFolder); itr != end_itr; ++itr)
		{
			...
			{
				if (boost::algorithm::iequals(itr->path().extension().string(), ".ecl_ws"))
				{
					StlLinked<CWorkspace> workspace = AttachWorkspaceRaw(repository, itr->path().stem().string(), true, false, attributesOnly);
		...
		if (!boost::filesystem::is_directory(*itr))
		{
			if (boost::algorithm::iequals(itr->path().stem().wstring(), label))
			{
				try {
					...
					_DBGLOG(LEVEL_WARNING, ex.what());
					_DBGLOG(LEVEL_WARNING, itr->path().native().c_str());
				}
			}
		}
	}
