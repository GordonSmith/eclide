...
void CPersistWindow::SetPath(const std::string & path, const boost::filesystem::path & folder)
{
	...
	else
	{
		...
		if (!(boost::filesystem::exists(m_filePath) && m_file.Open(m_filePath.native().c_str(), GENERIC_WRITE)))
			GenerateTmpFile(folder);
	}
...
void CPersistWindow::Restore()
{
	...
	CPersistMap persistInfo;
	m_file.Close();
	if (persistInfo.deserialize(m_filePath.native()))
	{
		WINDOWPLACEMENT placement;
	...
	m_file.Open(m_filePath.native().c_str(), GENERIC_WRITE);
}
