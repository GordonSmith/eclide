...
public:
	...
	void CheckForModuleFolders(std::_tstring & path)
	{
		...
		if (!hasChildFolder)
		{
			if (MessageBox((boost::_tformat(_T("The selected folder contains no \"Child\" folders, use %1% instead?")) % folder.parent_path().native()).str().c_str(), _T("AMT"), MB_YESNO) == IDYES)
				path = folder.parent_path().native();
		}
	}
...
bool PopulateConfigCombo(CComboBox &configCombo, const std::_tstring & defaultValue)
{
	...
	for (boost::filesystem::directory_iterator itr(folder); itr != end_itr; ++itr)
	{
		...
		{
			if (boost::algorithm::iequals(itr->path().extension().string(), ".cfg"))
			{
