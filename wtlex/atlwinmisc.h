...
#ifdef THIS_IS_NOW_IN_WTL_ATLUSER_H
/////////////////////////////////////////////////////////////////////////////
// CResource
...
public:
   ...
   {
      ATLASSERT(!::IsBadStringPtr(pstrSrc, (unsigned)-1));
      if( m_pstr ) delete [] m_pstr;
...
public:
	CLockWindowUpdate(HWND hWnd)
	{
		...
		ATLASSERT(::IsWindow(hWnd));
		m_locked = ::LockWindowUpdate(hWnd);
	}
...
public:
   ...
   { 
      ATLASSERT(!::IsBadStringPtrA(pszFuncName,(unsigned)-1));
      ATLASSERT(IsLoaded()); 
...
// to add a CComModule member just to be able to use _CSTRING_NS::CString and other
// WTL classes.
...
public:
   ...
   {
      ATLASSERT(!::IsBadStringPtr(pstr, (unsigned)-1));
   ...
   {
      ATLASSERT(hReg);
      ATLASSERT(!::IsBadStringPtr(pstrKeyName,(unsigned)-1));
      TCHAR szData[80];
   ...
   {
      ATLASSERT(!::IsBadStringPtr(pstrValueName,(unsigned)-1));
      ATLASSERT(hReg);
      TCHAR szData[80];
...
public:
   ...
   {
      ATLASSERT(!::IsBadStringPtr(pstrFilename,(unsigned)-1));
   ...
   BOOL GetString(LPCTSTR pstrSection, LPCTSTR pstrKey, LPTSTR pstrValue, UINT cchMax, LPCTSTR pstrDefault = NULL) const
   {
      ...
      ATLASSERT(!::IsBadStringPtr(pstrSection,(unsigned)-1));
      ATLASSERT(!::IsBadStringPtr(pstrKey,(unsigned)-1));
...
inline _CSTRING_NS::CString AtlLoadHTML(_U_STRINGorID html)
{
#if (_ATL_VER >= 0x0700)
